...
import { buildCompletedLifecycle, buildTransactionLifecycle, sleep } from "@/lib/transactionLifecycle";
...
export default function BrowseOrdersPage() {
    ...
    const confirmTakeOrder = async (order: Order) => {
        ...
        try {
            ...
            await sleep(800);
            updateTransaction(txId, { hash, status: TransactionStatus.CONFIRMING, lifecycle: buildTransactionLifecycle(chain, "broadcast") });
            await sleep(900);
            updateTransaction(txId, { status: TransactionStatus.CONFIRMING, lifecycle: buildTransactionLifecycle(chain, "confirm") });
            await sleep(1100);
            updateTransaction(txId, { status: TransactionStatus.COMPLETED, confirmations: 1, proofVerified: true, lifecycle: buildCompletedLifecycle(chain) });

            updateOrder(order.id, { status: OrderStatus.FILLED });
            setToasts([{ id: Date.now().toString(), type: "success", title: "Order Executed", message: `Successfully swapped ${order.pair}` }]);
        } catch (e) {
            setToasts([{ id: Date.now().toString(), type: "error", title: "Execution Failed", message: "Transaction could not be completed." }]);
        } finally {
            ...
            <div className="mb-10 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
                <div>
                    ...
                    <p className="mt-4 max-w-2xl text-lg leading-7 text-text-secondary">
                        Discover and execute cross-chain liquidity across all supported networks.
                        Filter by asset, chain, or status to find the best swap opportunities.
                    </p>
                </div>
                ...
                <div className="space-y-4">
                    <div className="flex items-center justify-between px-2">
                        <div className="flex items-center gap-3 text-sm text-text-secondary">
                            ...
                            <span>
                                Found {filtered.length} matching orders
                            </span>
                        </div>
                    </div>
                    ...
                    <div className="mt-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                        <div className="flex items-center gap-3 text-sm text-text-secondary">
                            ...
                            <span>
                                Showing {visibleOrders.length} orders per page
                            </span>
                        </div>
                        <PaginationControls
